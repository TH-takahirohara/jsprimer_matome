...
## 数値を整数にする
- Mathオブジェクトには数値を整数に丸めるためのメソッドがいくつかあります。 代表的なものは、小数点以下を切り捨てるMath.floorメソッド、小数点以下を切り上げるMath.ceilメソッド、そして四捨五入するMath.roundメソッドです。
...
- 合意が取れる最低限の形でプロポーザルを進めていく

## プロポーザルの機能を試す

## 仕様や策定プロセスを知る意味
- ECMAScriptという仕様や策定プロセスを知る意味は下記。
    - 言語を学ぶため
    - 言語が進化しているため
        - あるプロポーザルがECMAScriptにマージされ利用できる場合、その機能が何を解決するために導入されたのかを知ることが大切です。 
    - 情報の正しい状態を調べるため
        - ECMAScriptの仕様やその策定中のプロポーザルに関する情報は状態が明確です。
